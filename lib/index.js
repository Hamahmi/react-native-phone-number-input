...
import CountryPicker, {
  ...
  Flag,
} from "@hamahmi/react-native-country-picker-modal";
...
export default class PhoneInput extends PureComponent {
  ...
  componentDidUpdate = async (prevProps) => {
    if(prevProps.defaultValue !== this.props.defaultValue && this.props.defaultValue) {
      this.setState({
        number: this.props.defaultValue.toString()
      })
    }

    if(prevProps.defaultCode !== this.props.defaultCode && this.props.defaultCode) {
      const { defaultCode } = this.props;
      if (defaultCode) {
  ...
  getNumberType = (number) => {
   ...
   getNationalNumberAndRegionCode = (number) => {
    try {
      const parsedNumber = phoneUtil.parseAndKeepRawInput(number);
      const nationalNumber =  parsedNumber.getNationalNumber();
      const countryISOCode = phoneUtil.getRegionCodeForNumber(parsedNumber)
...
export const getNationalNumberAndRegionCode = (number) => {
  try {
    const parsedNumber = phoneUtil.parseAndKeepRawInput(number);
    const nationalNumber =  parsedNumber.getNationalNumber();
    const countryISOCode = phoneUtil.getRegionCodeForNumber(parsedNumber)
